  ...
  for (col in cat_cols) {
    p <- ggplot(df, aes(x = .data[[col]], fill = .data[[col]])) +
      ...
      labs(x = col, y = "Number of patients", 
           title = str_c("Bar plot of ", col, "for heart disease")) +
...
prob.prediction <- function(yhat.model) {
  as_tibble(yhat.model) %>%
    mutate(prob = if_else(absent > present, absent, present)) %>%
...
```{r model building_1, message= F, comment= F, warning= F}
...
```{r model building_2, message= F, comment= F, warning= F}
...
```{r model building_3, message= F, comment= F, warning= F}
...
The Github repository can be found [here](https://github.com/gbganalyst/Predicting-Heart-Disease) and if you like this writeup, you can also follow me on [Twitter](https://www.twitter.com/gbganalyst){target="_blank"} and [Linkedin](https://www.linkedin.com/in/ezekiel-ogundepo/){target="_blank"} for more updates in `R` and `Python` for datascience.
